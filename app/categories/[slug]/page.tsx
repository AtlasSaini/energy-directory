...
export default async function CategoryPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {/* Hero */}
      <div className="bg-[#1D1D1F] text-white rounded-2xl p-8 mb-8">
        <h1 className="text-3xl font-bold mb-2">{category.name}</h1>
        {category.description && (
          <p className="text-gray-300 max-w-2xl">{category.description}</p>
        )}
        <p className="text-[#E8590C] mt-2 text-sm font-medium">
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <aside className="lg:w-56 flex-shrink-0">
          <div className="bg-white border border-gray-200 rounded-xl p-4">
            <h2 className="font-semibold text-[#1D1D1F] text-sm mb-3">All Categories</h2>
            <div className="space-y-1">
              {allCategories.map((cat) => (
                <Link
                  key={cat.id}
                  href={`/categories/${cat.slug}`}
                  className={`block text-sm px-3 py-1.5 rounded-lg transition-colors ${cat.slug === slug ? 'bg-[#1D1D1F] text-white' : 'text-gray-700 hover:bg-gray-100'}`}
                >
                  {cat.name}
                </Link>
              ))}
        ...
        <div className="flex-1 min-w-0">
          {vendors.length > 0 ? (
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-12 text-center">
              ...
              <h3 className="text-lg font-semibold text-[#1D1D1F] mb-2">No vendors yet</h3>
